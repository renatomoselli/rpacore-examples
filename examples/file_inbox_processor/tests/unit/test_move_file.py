...
def _run(data, config):
    tx = Transaction(
        reference="move-file",
        steps=[MoveFile(name="move_file", execution_order=1)],
    )
...
def test_raises_when_no_source(tmp_path):
    ...
    assert "No source file available" in str(tx.failed_steps()[0].exceptions[-1])
...
def test_raises_when_no_done_dir(tmp_path):
    ...
    assert tx.status == Status.FAILED
    assert "done_dir" in str(tx.failed_steps()[0].exceptions[-1]).lower()
