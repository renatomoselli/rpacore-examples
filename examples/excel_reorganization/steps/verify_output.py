...
This step verifies that the output Excel file can be read with openpyxl,
...
Pattern: Follows examples/json_event_log_processor/steps/validate_events.py:3-64
"""
...
from rpacore import BusinessException, ProcessContext, Step, get_logger
...
class VerifyOutput(Step):
    ...
    def execute(self, ctx: ProcessContext) -> None:
        ...
        try:
            ...
            raise BusinessException(f"Cannot read output file: {exc}", action=self.name, halts_remaining_steps=True) from exc
    ...
    def _fail(self, message: str) -> None:
        ...
        raise BusinessException(message, action=self.name, halts_remaining_steps=True)
