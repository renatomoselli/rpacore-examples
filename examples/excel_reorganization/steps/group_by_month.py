...
This step groups rows from ctx.state["sales_data"] by year-month key (YYYY-MM),
sorts each group by employee name, and stores the grouped data in
ctx.state["grouped_data"] as a dict[str, list[dict]].

Pattern: Follows examples/json_event_log_processor/steps/load_json_file.py:52
"""

from __future__ import annotations
import datetime
from typing import Any
from rpacore import BusinessException, ProcessContext, Step, SystemException, get_logger

logger = get_logger(__name__)


class GroupByMonth(Step):
    """Group sales data by year-month and sort by employee name."""
    ...
    def execute(self, ctx: ProcessContext) -> None:
        ...
        for i, row in enumerate(sales_data):
            ...
            if not date_str:
                raise BusinessException(f"Row {i + 1} missing date field", action=self.name, halts_remaining_steps=True)
            try:
        ...
        # Set expected months for VerifyOutput step. Keep durable state JSON-safe.
