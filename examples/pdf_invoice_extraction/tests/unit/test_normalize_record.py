"""Unit tests for NormalizeRecord step."""
...
import steps.normalize_record as normalize_record_module
from steps.normalize_record import NormalizeRecord

class TestNormalizeRecord:
    """Tests for NormalizeRecord step."""
    ...
    def _run_step(self, **state_overrides):
        parsed = state_overrides.pop("parsed_invoice", self._make_parsed_invoice())
        tx = Transaction(reference="test", steps=[NormalizeRecord(name="normalize_record", execution_order=1)])
        tx.state["parsed_invoice"] = parsed
        tx.state.update(state_overrides)
        ctx = ProcessContext(transaction=tx, config={})
        step = NormalizeRecord(name="normalize_record", execution_order=1)
        step.execute(ctx)
        return tx
    ...
    def test_normalize_record_rounding(self):
        tx = self._run_step(parsed_invoice=self._make_parsed_invoice(total=250.127, subtotal=250.123))
    ...
    def test_normalize_record_vendor_uppercase(self):
        tx = self._run_step(parsed_invoice=self._make_parsed_invoice(vendor="Euro Parts GmbH"))
    ...
    def test_normalize_record_none_total_preserved(self):
        tx = self._run_step(parsed_invoice=self._make_parsed_invoice(total=None, subtotal=None))
