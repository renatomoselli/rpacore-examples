"""Unit tests for ValidateInvoice step."""
...
class TestValidateInvoice:
    """Tests for ValidateInvoice step."""
    ...
    def _run_step(self, parsed_invoice=None):
        tx = Transaction(reference="test", steps=[ValidateInvoice(name="validate_invoice", execution_order=1)])
        if parsed_invoice is not None:
            tx.state["parsed_invoice"] = parsed_invoice
        ctx = ProcessContext(transaction=tx, config={})
        step = ValidateInvoice(name="validate_invoice", execution_order=1)
        step.execute(ctx)
        return tx

    def test_validate_invoice_passes(self):
        tx = self._run_step(self._make_parsed_invoice())
    ...
    def test_validate_invoice_total_within_tolerance(self):
        tx = self._run_step(self._make_parsed_invoice(total=250.03))
        assert tx.state.get("validation_failed") is False
