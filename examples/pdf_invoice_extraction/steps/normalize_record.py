...
from rpacore import ProcessContext, Step, Status, SystemException, get_logger

from steps._currency import try_parse_currency_number

logger = get_logger(__name__)

class NormalizeRecord(Step):
    """Normalize parsed invoice data for consistent CSV output.

    Checks for validation failures first (defensive backstop):
    if validate_invoice set validation_failed=True, this step
    ...
    def execute(self, ctx: ProcessContext) -> None:
        ...
        # Defensive backstop on validation failure (BusinessException(halts_remaining_steps=True)
