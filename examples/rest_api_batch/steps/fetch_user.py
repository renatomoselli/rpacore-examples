...
from steps import API_MODE_FIXTURE, API_MODES, fetch_json
    ...
    def execute(self, ctx: ProcessContext) -> None:
        ...
        if type(user_id) is not int or user_id <= 0:
            raise BusinessException(
                ...
                action=self.name, halts_remaining_steps=True, code="rest_api_batch.user.invalid",
            )
        ...
        if api_mode == API_MODE_FIXTURE:
            ...
            if user is None:
                raise BusinessException(
                    ...
                    action=self.name, halts_remaining_steps=True, code="rest_api_batch.user.not_found",
                )
