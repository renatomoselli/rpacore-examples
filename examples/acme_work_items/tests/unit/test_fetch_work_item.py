...
from skills._session import RemoteWorkItem
...
def test_fetch_records_json_safe_remote_snapshot(monkeypatch, example_config) -> None:
    monkeypatch.setattr("skills.fetch_work_item.require_authenticated_session", lambda ctx: FetchSession())
...
def test_fetch_sanitizes_unexpected_failures(monkeypatch, example_config) -> None:
    ...
    monkeypatch.setattr("skills.fetch_work_item.require_authenticated_session", fail)
    transaction = run_skill(
        FetchWorkItem(name="fetch", execution_order=1),
        state={"work_item_id": "1001"},
        config=example_config,
